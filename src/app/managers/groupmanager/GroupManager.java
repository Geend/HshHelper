...
public class GroupManager {
    ...
    public void createGroup(String groupName) throws GroupNameAlreadyExistsException, InvalidArgumentException {
        ...
        try(Transaction tx = ebeanServer.beginTransaction(TxIsolation.REPEATABLE_READ)) {
            ...
            if(txGroup.isPresent()) {
                logger.error(user + " (userid " + user.getUserId() + ") tried to create group" + groupName + " but that name already exists");
            ...
            tx.commit();
            logger.info(user + " (userid " + user.getUserId() + ") created group" + groupName);
        }
    }
    ...
    public List<Group> getAllGroups() throws InvalidArgumentException, UnauthorizedException {
        ...
        logger.info(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") is looking at all groups.");
    ...
    public Group getGroup(Long groupId) throws InvalidArgumentException, UnauthorizedException {
        ...
        if(!sessionManager.currentPolicy().canViewGroupDetails(group)) {
            logger.error(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") tried to access group " + group.getName() + " for which he is not authorized");
    ...
    public List<User> getUsersWhichAreNotInThisGroup(Long groupId) throws UnauthorizedException, InvalidArgumentException {
        ...
        if (!sessionManager.currentPolicy().canViewGroupDetails(group)) {
            logger.error(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") tried to see the members of group " + group.getName() + " for which he is not authorized");
    ...
    public void removeGroupMember(Long userToRemove, Long groupId) throws UnauthorizedException, InvalidArgumentException {
        ...
        if(!sessionManager.currentPolicy().canRemoveGroupMember(g, toBeRemovedUser)) {
            logger.error(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") tried to delete + " + toBeRemovedUser.getUsername() + " from group " + g.getName() + " but he is not authorized");
        ...
        logger.info(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") deleted " + toBeRemovedUser.getUsername() + " from group " + g.getName());
    }
    ...
    public void addGroupMember(Long userToAdd, Long groupId) throws UnauthorizedException, InvalidArgumentException {
        ...
        if (!sessionManager.currentPolicy().canAddSpecificGroupMember(g, toBeAddedUser)) {
            logger.error(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") tried to add + " + toBeAddedUser.getUsername() + " to group " + g.getName() + " but he is not authorized");
        ...
        logger.info(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") added " + toBeAddedUser.getUsername() + " to group " + g.getName());
    }
    ...
    public void deleteGroup(Long groupId) throws UnauthorizedException, InvalidArgumentException {
        ...
        if (!sessionManager.currentPolicy().canDeleteGroup(g)) {
            logger.error(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") tried to delete group " + g.getName() + " but he is not authorized");
            throw new UnauthorizedException("Du bist nicht authorisiert, eine Gruppe zu loeschen.");
        }

        ebeanServer.delete(g);
        logger.info(sessionManager.currentUser().getUsername() + " (userid " + sessionManager.currentUser().getUserId() + ") deleted group " + g.getName());
    }
