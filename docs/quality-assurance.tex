...
In der Oberfläche wird der Zeilenumbruch wie erwartet dargestellt. Beim Versuch die Datei herunterzaladen gibt es eine Exception: \texttt{[ServerResultException: Error converting Play Result for server backend]}. Der Benutzer bekommt eine Fehlerseite mit dem HTTP Statuscode 500 angezeigt und die Datei wird nicht heruntergeladen. Weiter Seiteneffekte konnten dabei nicht beobachtet werden.
