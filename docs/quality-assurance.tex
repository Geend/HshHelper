...
Für eine Play-Session wird nur ein Token generiert (aus Listing \ref{csrfGenerateToken}). Dieses ist solange gültig wie die Play-Session und ändert sich nicht. Es wird für jedes Formular innerhalb einer Session verwendet. Da die Nonce auf der Systemzeit basiert, ändert sich diese jedoch mit jedem Request, was zu unterschiedlichen Signaturen führt. Dadurch werden Attacken, die ähnlich wie \texttt{BREACH} die HTTP-Komprimierung bei Verwendung von HTTPS angreift, verhindert\footnote{Siehe dazu https://www.playframework.com/documentation/2.6.x/JavaCsrf unten}.
