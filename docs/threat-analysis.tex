...
\begin{table*}[hbt!]
  ...
  \textbf{Gegenmaßnahme} & \textbf{Angriff} & \textbf{Status}\\ \hline
    Eingabevalidierung & \linktothreat{threat6}{T6}, \linktothreat{threat7}{T7}, \linktothreat{threat15}{T15} & \greencheckmark \\ \hline
...
\end{sloppypar}
Durch den Header \texttt{X-XSS-Protection} wird der Browser (Chrome, Safari, Internet Explorer und Opera) angewiesen, eine \enquote{reflected XSS}-Erkennung zu versuchen. Dieses Feature ist für älteren Browser interessant, die noch kein CSP implementieren. Durch den Modus \enquote{block} wird das Laden der Seite abgebrochen.
...
Solch ein Blacklisting ist allerdings aufgrund verschiedener Möglichkeiten zum Deformieren des Payloads ziemlich unsicher.
Dieser Header dient daher als zusätzliches Auffangnetz, aber nicht als ausreichender Schutz.
\footnote{Siehe \url{https://blog.innerht.ml/the-misunderstood-x-xss-protection/}}
