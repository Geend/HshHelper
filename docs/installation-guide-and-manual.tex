...
.2 // Alle Abhängigkeiten.
}

Im \texttt{bin}-Ordner befinden sich Startskripte für Windows sowie UNIX.
Diese können auf der Kommandozeile ausgeführt werden, um die HsH-Helfer Applikation zu starten.

Nach dem Start der Applikation werden Sie auf der Kommandozeile von einigen Logging-Ausgaben begrüßt.
Sobald Sie die Zeile \texttt{[info] p.c.s.AkkaHttpServer - Listening for HTTP on /0:0:0:0:0:0:0:0:9000} sehen, ist der Server betriebsbereit.
Danach können sie den HsH-Helfer von ihrem Browser erreichen, in dem sie \texttt{\url{http://localhost:9000}} ansurfen, sofern sie die IP-Adresse / Hostname sowie Port nicht verändert haben (siehe \autoref{manual:configuration})

Während des Hochfahrens der Applikation wird ein Ordner \texttt{logs/} in dem Applikationsverzeichnis erstellt.
...
Sollten sie die IP-Adresse oder den Port ändern wollen, auf dem der HsH-Helfer standardmäßig läuft, so müssen sie das beim Start der Applikation angeben, in dem sie folgende Parameter auf der Kommandozeile hinzufügen: \\
...
Sollten Sie den HsH-Helfer auf einer externen Domäne anbieten wollen, so müssen Sie in der Konfigurationsdatei folgende Einstellungen ändern, damit die Applikation auch funktional korrekt weiterläuft.
...
\begin{lstlisting}[label=server-foreign-domain, caption={},captionpos=b]
play.filters {
  headers {
...
\begin{lstlisting}[label=server-foreign-allowed-hosts, caption={},captionpos=b]
play.filters {
  hosts {
